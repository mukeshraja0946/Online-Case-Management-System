...
switch ($range) {
    ...
    case '1month':
        ...
        for ($i = $days; $i >= 0; $i--) {
            ...
            foreach ($case_types as $type) {
                $sql = "SELECT COUNT(*) as count FROM cases WHERE DATE(incident_date) = ? AND case_type = ? AND deleted_by_staff = 0";
                $stmt = $conn->prepare($sql);
    ...
    case '1year':
        ...
        for ($i = $months; $i >= 0; $i--) {
            ...
            foreach ($case_types as $type) {
                $sql = "SELECT COUNT(*) as count FROM cases 
                        WHERE MONTH(incident_date) = ? 
                        AND YEAR(incident_date) = ? 
                        AND case_type = ? 
                        AND deleted_by_staff = 0";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("iis", $month_val, $year_val, $type);
                $stmt->execute();
                $count = $stmt->get_result()->fetch_assoc()['count'];
